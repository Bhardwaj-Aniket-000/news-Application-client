...
const Cloud = () => {
  ...
  let cloudShowTime ;
  if(weatherinfo.time){
      cloudShowTime = weatherinfo.time.slice(10).split("")[1]+weatherinfo.time.slice(10).split("")[2]
  }else{
  ...
  return (
    ...
    <div className='w-[80%] p-4 pl-8 flex text-black gap-4 flex-col'>
        ...
        <h1 className=' text-xl font-bold text-red-700 uppercase underline'>clouds</h1>
            ...
            <div className='flex items-start flex-col mt-2'>
              ...
              <div className='flex items-center mt-2'>
                {cloudShowTime > 7 && cloudShowTime < 19 ? <img src={cloudDay} alt="current cloudy icon" className='w-10 mt-2'/> : <img src={cloudnight} alt="current cloudy icon" className='w-10 mt-2'/>}
                <p className='text-3xl font-semibold'>{weatherinfo.Cloud}%</p>
              </div>
            </div>
            <p className='capitalize font-medium text-sm'>{weatherinfo.Cloud}% , cloudy at time</p>
        </div>
                ...
                <div key={ind}>
                  <div className='flex flex-col gap-2 w-[90%] rounded-md bg-slate-200 p-2'>
                      ...
                      <div className='flex flex-col items-center'>
                        {time > 7 && time < 19 ? <img src={cloudDay} alt="cloudy-icon" className='w-12 mt-2'/> : <img src={cloudnight} alt="cloudy-icon" className='w-12 mt-2'/>}
                        <p className="text-center text-sm font-medium">{obj.cloud}% <span className='text-xs'>cloudy</span></p>
                      </div>
                  </div>
                </div>
              )
